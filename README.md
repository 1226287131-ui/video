...
`MiniMax-H3-933-1440P-GF` 使用 `/v1/videos` 异步接口。网页请求只发送 `model`、`prompt`、`seconds`（4-15 秒）、`size` 和已选择的参考素材；`audio`、`prompt_enhance`、`clarity`、`megapixels`、`metadata.multiple` 等高级字段一律省略，由上游使用默认值。参考素材使用公网 URL：`images` 最多 9 张、`reference_videos` 最多 3 个、`reference_video_audios` 最多 3 个、`reference_audios` 最多 3 个；相同 URL 或文件名会去重。前端会先把用户上传的文件暂存到项目域名，过期后由上传服务自动清理，不把视频文件落到站点目录。
